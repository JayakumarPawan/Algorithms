...
def get_most_constrained_position(CONSTRAINTS):
    ...
    for constraint, prefix in open_constraints.items():
        # print(prefix, len(PREFIXES[prefix]))
...
def find_options_for_all_locations(pzl, used, CONSTRAINTS):
    ...
    # print('prefix', prefix, 'existing', CONSTRAINTS[constraint])
    # CONSTRAINTS[constraint] = prefix
    # print(f'looking for words {len(prefix)} long')
    if prefix in PREFIXES:
        # print('stored some already', prefix,PREFIXES[prefix])
        cache_used += 1
